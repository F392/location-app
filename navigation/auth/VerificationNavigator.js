...
const VerificationNavigator = ({ route, navigation }) => {
  ...
  const handleSendEmailAgain = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost/api/send_mail_again",
        {
          email,
        }
      );
  ...
  const handleCancel = async () => {
    try {
      const response = await axios.post(
        "http://localhost/api/user/delete_temp_user",
        {
          email,
        }
      );
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>{email} に送信された6桁のコード</Text>
